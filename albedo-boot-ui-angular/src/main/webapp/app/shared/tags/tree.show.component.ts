...
@Component({
    selector: "alb-tree-show",
    templateUrl: "./tree.show.component.html"
})
export class AlbTreeShowComponent implements OnInit, AfterViewInit {

    @Input() id?: string;
    @Input() title?: string; //标题
    @Input() checked?: string;  //是否显示复选框
    @Input() url?: string; // 树结构数据地址
    @Input() checkNodeFn?: string; //选中事件
    @Input() beforeCheckNodeFn?: string; //菜单checkbox / radio被勾选 或 取消勾选之前的事件回调函数，并且根据返回值确定是否允许 勾选 或 取消勾选
    @Input() cancelClickNodeFn?: string;//菜单节点取消点击的事件回调函数
    @Input() clickNodeFn?: string; //菜单节点被点击的事件回调函数
    @Input() afterLoadNodeFn?: string; //菜单加载完毕回调函数
