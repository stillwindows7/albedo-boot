...
export class TestBookComponent implements AfterViewInit {
    ...
    initTable() {
                ...
                template: function(row) {
                    ...
                    if (thisPrincipal.hasAnyAuthorityDirectOne("test_testBook_edit"))
                        template += '<a href="javascript:void(0)" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill dialog-edit" title="编辑"\
                            \data-method="get"  data-title="编辑【' + row.name + '】" data-url="' + CTX + '/test/testBook/' + row.id + '" data-modal-id="#testBook-edit-modal">\
                    ...
                    if (thisPrincipal.hasAnyAuthorityDirectOne("test_testBook_lock"))
                     ...
                     data-table-id="#data-table-testBook" data-method="put"  data-title="你确认要操作选中的测试书籍吗？" data-url="' + CTX + '/test/testBook/' + row.id + '">\
                    ...
                    if (thisPrincipal.hasAnyAuthorityDirectOne("test_testBook_delete"))
                        template += '<a  href="javascript:void(0)" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill confirm" title="删除"\
                               data-table-id="#data-table-testBook" data-method="delete"  data-title="你确认要删除选中的测试书籍吗？" data-url="' + CTX + '/test/testBook/' + row.id + '">\
        ...
        albedoList.init();
        albedoForm.init();
        albedoForm.initSave($("#testBook-edit-modal"));
    }
