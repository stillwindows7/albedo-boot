            ...
            columns: [
					{title: '名称',field:'name'
					 ,width: 110,sortable: 'asc',overflow: 'visible',template: function(row) {
                        return thisPrincipal.hasAuthority("sys_taskScheduleJob_edit") ? ( '<a href="#/sys/taskScheduleJob/form/' + row.id + '" class="m-link" title="点击编辑任务调度">'+row.name+'</a>') : row.name;
                    },},
        ...
        if(thisPrincipal.hasAnyAuthorityDirect(["sys_taskScheduleJob_edit","sys_taskScheduleJob_lock","sys_taskScheduleJob_delete"])){
            options.columns.push({
                ...
                template: function(row) {
                    var template = '';
                    if (thisPrincipal.hasAuthority("sys_taskScheduleJob_edit"))
                        template += '<a href="#/sys/taskScheduleJob/form/' + row.id + '" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="编辑">\
                            ...
                            \</a>';
                    if (thisPrincipal.hasAuthority("sys_taskScheduleJob_lock"))
                     ...
                     data-table-id="#data-table-taskScheduleJob" data-method="put"  data-title="你确认要操作【' + row.name + '】任务调度吗？" data-url="' + CTX + '/sys/taskScheduleJob/' + row.id + '">\
                            ...
                            \</a>';
                    if (thisPrincipal.hasAuthority("sys_taskScheduleJob_delete"))
