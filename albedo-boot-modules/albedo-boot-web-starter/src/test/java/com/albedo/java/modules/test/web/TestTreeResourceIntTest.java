...
public class TestTreeResourceIntTest {
	...
	private static final Date DEFAULT_DEFAULTDATA = PublicUtil.getCurrentDate();
	/** UPDATED_DEFAULTDATA default_data  :  默认日期 */
    private static final Date UPDATED_DEFAULTDATA = DateUtil.addDays(PublicUtil.getCurrentDate(), 1);
    ...
    @Test
    @Transactional
    public void getAllTestTreesBySortIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        testTreeRepository.saveAndFlush(testTree);

        // Get all the testTreeList where sort greater than or equals to DEFAULT_SORT
        defaultTestTreeShouldBeFound(QueryCondition.eq(TestTree.F_ID, testTree.getId()),
         QueryCondition.ge(TestTree.F_SORT, DEFAULT_SORT)
        );

        // Get all the testTreeList where sort greater than or equals to UPDATED_SORT
        defaultTestTreeShouldNotBeFound(QueryCondition.eq(TestTree.F_ID, testTree.getId()),
         QueryCondition.ge(TestTree.F_SORT, UPDATED_SORT)
        );
    }

    @Test
    @Transactional
    public void getAllTestTreesBySortIsLessThanSomething() throws Exception {
        // Initialize the database
        testTreeRepository.saveAndFlush(testTree);

        // Get all the testTreeList where sort less than or equals to DEFAULT_SORT
        defaultTestTreeShouldNotBeFound(QueryCondition.eq(TestTree.F_ID, testTree.getId()),
         QueryCondition.lt(TestTree.F_SORT, DEFAULT_SORT)
        );

        // Get all the testTreeList where sort less than or equals to UPDATED_SORT
        defaultTestTreeShouldBeFound(QueryCondition.eq(TestTree.F_ID, testTree.getId()),
         QueryCondition.lt(TestTree.F_SORT, UPDATED_SORT)
        );
    }
    @Test
    @Transactional
