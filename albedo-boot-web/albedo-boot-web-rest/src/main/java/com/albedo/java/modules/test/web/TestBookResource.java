...
public class TestBookResource extends DataVoResource<TestBookService, TestBookVo> {
	...
	@Timed
	public String list() {
		return "modules/test/testBookList";
	}
	...
	@Timed
	public String form(TestBookVo testBookVo) {
		if(testBookVo == null){
			throw new RuntimeMsgException(PublicUtil.toAppendStr("查询测试书籍失败，原因：无法查找到编号为[", request.getParameter("id"), "]的测试书籍"));
		}
		return "modules/test/testBookForm";
	}
	 ...
	 * @return the ResponseEntity with status 200 (OK)
	 */
	@PutMapping(value = "/lock/{ids:" + Globals.LOGIN_REGEX
			+ "}")
	@Timed
